...
void calc_chemical_potential(double *** sig00, double **** eps0, double ** chem, double ** eta, double *** eps00, double ** epsbar, double **** sig_temp, double *** eps, double ** lap, double * phi, double ** dw, ptrdiff_t local_n0, ptrdiff_t N1, struct input_parameters ip)
{
...
int main(int argc, char ** argv)
{
    ...
    initialize_w(w, local_n0, local_0_start, N1);
 ...
 /*           introduce_noise(eta, local_n0, N1);
            ...
            calc_chemical_potential(sig00, eps0, chem, eta, eps00, epsbar, sig_temp, eps, lap, phi, dw, local_n0, N1, ip);
        ...
        if ( step % ip.out_freq == 0) {
            ...
            output("eta1/"+zeroFill(frame), eta[1], N0, N1, local_n0);
            output("eta2/"+zeroFill(frame), eta[2], N0, N1, local_n0);
            output("eps_00/"+zeroFill(frame), eps[0][0], N0, N1, local_n0);
            output("eps_01/"+zeroFill(frame), eps[0][1], N0, N1, local_n0);
            output("eps_10/"+zeroFill(frame), eps[1][0], N0, N1, local_n0);
            output("eps_11/"+zeroFill(frame), eps[1][1], N0, N1, local_n0);
            output("w/"+zeroFill(frame), w, N0, N1, local_n0);
