...
void calc_uxy_bending(double * ux, double * uy, fftw_complex ** ku, double ** dw, double ** ddw,
                      ...
                      ptrdiff_t N0, ptrdiff_t N1, ptrdiff_t local_n0, ptrdiff_t local_0_start, double Radius)
{
    const int N1c = N1/2+1;
    const int N1r = 2*(N1/2+1);
    ...
    fftw_plan planF_ux = fftw_mpi_plan_dft_r2c_2d(N0, N1, ux, ku[0], MPI_COMM_WORLD, FFTW_ESTIMATE);
    fftw_plan planF_uy = fftw_mpi_plan_dft_r2c_2d(N0, N1, uy, ku[1], MPI_COMM_WORLD, FFTW_ESTIMATE);
...
/*    double r;
   ...
   fftw_execute(planF_ux);
   fftw_execute(planF_uy);
   fftw_destroy_plan(planF_ux);
   fftw_destroy_plan(planF_uy);
}
...
////////////////////////////////////////////////////////////////////////////////////////////
void calc_eps(double *** eps_out, double ** dw, double *** eps, fftw_complex ** keps, 
...
////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for(int ii=0;ii<2;ii++)
    for(int jj=0;jj<2;jj++)
    eps_out[ii][jj][ndx] = eps[ii][jj][ndx] + 0.5*dw[ii][ndx]*dw[jj][ndx];   
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
double update_w(double * w, double * w_old, double * w_new, double * dFdw, ptrdiff_t local_n0, ptrdiff_t N1, double alpha_w, double gamma_w, double dt)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for (int j=0; j<N1; j++)
    {
        ...
        //  w_new[ndx] = -dt*dFdw[ndx]*gamma_w + w[ndx];
        w_old[ndx] = w[ndx];
...
void indenter(double * w, double depth, double Radius, ptrdiff_t local_n0, ptrdiff_t local_0_start, ptrdiff_t N1)
{
    double r;
   for (int i=0; i<local_n0; i++)
    for (int j=0; j<N1; j++)
    {
        ...
        w[ndx] = 0;  
       /* w[ndx] = fabs(x -local_n0/2)/Radius*depth - depth;
        if(w[ndx]>0)
        w[ndx] = 0;  */
}    
}
...
int main(int argc, char ** argv)
{
    ...
    double *** eps;     // heterogeneous strain, no out of plane terms     eps[p][i][j][ndx]
    double *** eps_out; // heterogeneous strain with out of plane terms
    ...
    eps_out     = (double ***)  kd_alloc2(sizeof(double), 3, 2, 2, 2*alloc_local);
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
        ...
        double depth = (step/(double)ip.nsteps)*15;
        ...
        while (change_etap_max > ip.change_etap_thresh)
        {
          ...
          while (change_eps00_max > ip.change_eps00_thresh)
           { 
            ...
            calc_uxy_bending(ux, uy, ku, dw, ddw, lam0, G, kxy, ksig00, N0, N1, local_n0, local_0_start, Radius);

            // calculate the heterogeneous strain (delta-epsilon) in k-space
            calc_eps(eps_out, dw, eps, keps, kxy, ku, N0, N1, local_n0);
...
/*            calc_dNuterm( kappa, nu, Nuterm, ddNuterm, kxy, phi, N0, N1, local_n0, ip);
            ...
            change_w_max = update_w(w, w_old, w_new, dFdw, local_n0, N1, ip.alpha_w, ip.gamma_w, ip.dt);
            add_w_noise(w, local_n0, N1);
*/
            indenter(w, depth, Radius, local_n0, local_0_start, N1);

            std::cout << "w = " << max_w(w, local_n0, N1) << "    cwmax = "<< change_w_max << std::endl;     
        ...
        if ( step % ip.out_freq == 0) {
            ...
            output("eta1/"+zeroFill(frame), eta[1], N0, N1, local_n0);
            output("eta2/"+zeroFill(frame), eta[2], N0, N1, local_n0);
            output("eps_out00/"+zeroFill(frame), eps_out[0][0], N0, N1, local_n0);
            output("eps_out01/"+zeroFill(frame), eps_out[0][1], N0, N1, local_n0);
            output("eps_out10/"+zeroFill(frame), eps_out[1][0], N0, N1, local_n0);
            output("eps_out11/"+zeroFill(frame), eps_out[1][1], N0, N1, local_n0);
            output("eps00/"+zeroFill(frame), eps[0][0], N0, N1, local_n0);
            output("eps01/"+zeroFill(frame), eps[0][1], N0, N1, local_n0);
            output("eps10/"+zeroFill(frame), eps[1][0], N0, N1, local_n0);
            output("eps11/"+zeroFill(frame), eps[1][1], N0, N1, local_n0);
            output("w/"+zeroFill(frame), w, N0, N1, local_n0);
            output("dw0/"+zeroFill(frame), dw[0], N0, N1, local_n0);
            output("dw1/"+zeroFill(frame), dw[1], N0, N1, local_n0);
