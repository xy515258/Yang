...
void initialize_lsf_stripe(double * lsf, ptrdiff_t local_n0, ptrdiff_t local_0_start, ptrdiff_t N1)
{
    ...
    for (int j=0; j<N1; j++)
    {
        ...
        lsf[ndx] = 1;
        if (x > 50 && x < 150) lsf[ndx] = -1;
    }
}

void initialize_lsf_stripe_eta0(double *lsf, ptrdiff_t local_n0, ptrdiff_t local_0_start, ptrdiff_t N1)
{
 for (int i=0; i<local_n0; i++)
    for (int j=0; j<N1; j++)
    {
        ...
        lsf[ndx] = -1;
        if ((x > 50 && x < 75)||(x > 125 && x < 150) ) lsf[ndx] = 1;
    }
}

void initialize_lsf_stripe_eta1(double *lsf, ptrdiff_t local_n0, ptrdiff_t local_0_start, ptrdiff_t N1)
{
 for (int i=0; i<local_n0; i++)
    for (int j=0; j<N1; j++)
    {
