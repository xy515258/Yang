...
void set_transformation_strains(double **** epsT)
{   
    const int M0 = 0;
    const int M1 = 1;
    for (int mat=M0; mat<=M1; mat++)
...
int main(int argc, char ** argv)
{
    ...
    //set_transformation_strains(epsT);  #For test use only
    ...
    initialize_lsf_stripe_eta0(lsf, local_n0, local_0_start, N1);
    diffuse_lsf(lsf, local_n0, N1);
    copy_lsf(lsf, eta[0], local_n0, N1);
    initialize_lsf_stripe_eta1(lsf, local_n0, local_0_start, N1);
    diffuse_lsf(lsf, local_n0, N1);
    copy_lsf(lsf, eta[1], local_n0, N1);
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
       ...
       {epsbar[0][0] = ip.epsx * ((step-2000)/(double)(ip.nsteps-2000));
        epsbar[1][1] = ip.epsy * ((step-2000)/(double)(ip.nsteps-2000));
        epsbar[0][1] = 0;
        epsbar[1][0] = 0;}
