...
void set_substrate(double * w_sub, double ind_x, double ind_y, double Radius, ptrdiff_t local_n0, ptrdiff_t local_0_start, ptrdiff_t N1)
{
    ...
    for (int j=0; j<N1; j++)
    {
        int ndx = i*2*(N1/2+1) + j;
        int x = local_0_start + i;
        double rx = fabs(x - ind_x);
        double ry = fabs(j - ind_y);
        if( (rx<=10)&&(ry<=10) )
        w_sub[ndx] = 2*Radius;
        else
        w_sub[ndx] = 0;  
...
int main(int argc, char ** argv)
{
    ...
    read_restart(360, w, dw, ddw, w_old, eta, eta_old, eps00, N0, N1, local_n0);
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
        ...
        double Radius = (step/(double)ip.nsteps)*10+1.8;
       ...
       if ((step == 350)||(step==300)||(step==360)||(step==370))      
