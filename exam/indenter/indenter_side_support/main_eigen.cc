...
double update_eta(double ** eta, double ** eta_old, double ** eta_new, double ** lap_deta, double ** chem, ptrdiff_t local_n0, ptrdiff_t N1, struct input_parameters ip)
{
    ...
    for (int j=0; j<N1; j++)
    {
        ...
        eta_new[p][ndx] = dtg2*(2*eta[p][ndx] + (dtg-1)*eta_old[p][ndx] + ip.dt*ip.gamma1*lap_deta[p][ndx] - dta2*chem[p][ndx]);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
double update_w(double ind_x, double ind_y, double * w, double * w_old, double * w_new, double * dFdw, ptrdiff_t local_n0, ptrdiff_t N1, double alpha_w, double gamma_w, double dt)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for (int j=0; j<N1; j++)
    {
        ...
        if( x!=ind_x && j!=ind_y && x!=0 && x!=N1 && j!=0 && j!=N1 )       
        delta = fabs( w_new[ndx] - w[ndx] );
        else
        delta = 0;
...
int main(int argc, char ** argv)
{
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
        ...
        double change_w_max =1;
        double area_fraction;
        
        while (change_etap_max > ip.change_etap_thresh || change_w_max > 0.001)
        {
            change_etap_max = 0;
            change_w_max = 0;
            change_eps00_max = 1;
          ...
          while (change_eps00_max > ip.change_eps00_thresh)
           { 
            ...
            change_w_max = update_w(ind_x, ind_y, w, w_old, w_new, dFdw, local_n0, N1, ip.alpha_w, ip.gamma_w, ip.dt);
