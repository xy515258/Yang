...
void write_restart(double step, double * w, double ** dw, double ** ddw, double * w_old, double ** eta, double ** eta_old, double *** eps00, double *** eps00_old, ptrdiff_t N0, ptrdiff_t N1, ptrdiff_t local_n0)
{
    ...
    output_restart("eps00_old[0][0]/"+zeroFill(step), eps00_old[0][0], N0, N1, local_n0);
    output_restart("eps00_old[0][1]/"+zeroFill(step), eps00_old[0][1], N0, N1, local_n0);
    output_restart("eps00_old[1][0]/"+zeroFill(step), eps00_old[1][0], N0, N1, local_n0);
    output_restart("eps00_old[1][1]/"+zeroFill(step), eps00_old[1][1], N0, N1, local_n0);
}

void read_restart(double step, double * w, double ** dw, double ** ddw, double * w_old, double ** eta, double ** eta_old, double *** eps00, double *** eps00_old, ptrdiff_t N0, ptrdiff_t N1, ptrdiff_t local_n0)
{
    ...
    input_restart("eps00_old[0][0]/"+zeroFill(step), eps00_old[0][0], N0, N1, local_n0);
    input_restart("eps00_old[0][1]/"+zeroFill(step), eps00_old[0][1], N0, N1, local_n0);
    input_restart("eps00_old[1][0]/"+zeroFill(step), eps00_old[1][0], N0, N1, local_n0);
    input_restart("eps00_old[1][1]/"+zeroFill(step), eps00_old[1][1], N0, N1, local_n0);
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for (int j=0; j<N1; j++)
    {
        ...
        w_new[ndx] = dtg2*(2*w[ndx] + (dtg-1)*w_old[ndx] - dta2*(dFdw[ndx]+10*dFdw_Pau[ndx]));
...
int main(int argc, char ** argv)
{
    ...
    //read_restart(1150, w, dw, ddw, w_old, eta, eta_old, eps00, eps00_old, N0, N1, local_n0);
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
        ...
        double depth = (step/(double)ip.nsteps)*30;
        ...
        while ((change_etap_max > ip.change_etap_thresh)||(change_w_max > ip.change_w_thresh))  //loop for out-of-plane displacement
        {
            change_etap_max = 0;
            change_eps00_max = 1;
            change_w_max = 0;
          ...
          while (change_eps00_max > ip.change_eps00_thresh)
           { 
            ...
            //myfile << step << "  cepmax=" << change_etap_max << "  ceps00max=" << change_eps00_max << "   Af=" << area_fraction << std::endl;
            ...
            // the rest for out-of-plane displacements - in progress
             ...
             //indenter(w_sub, ind_x, ind_y, depth, Radius, local_n0, local_0_start, N1);   //Intro indenter
            ...
            //myfile << "w = " << max_w(w, local_n0, N1) << "    cwmax = "<< change_w_max << "Radius=" << Radius << std::endl;     

            // calculate and output area - will change in future versions 
            area_fraction = calc_area(eta, local_n0, N0, N1, ip.M1_norm);
            
            std::cout << step << "  cepmax=" << change_etap_max << "  ceps00max=" << change_eps00_max << "   Af=" << area_fraction << std::endl;
            printf("%8d cepmax=%12.10f, ceps00max=%12.10f, Af=%12.10f\n",step,change_etap_max,change_eps00_max,area_fraction); 
        }
       ...
       if ( step % 100 == 0)      
       write_restart(step, w, dw, ddw, w_old, eta, eta_old, eps00, eps00_old, N0, N1, local_n0); 
